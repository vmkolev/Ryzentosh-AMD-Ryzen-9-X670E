...
### BIOS settings
After resetting BIOS to "default optimized" I've changed the following settings:
- iGPU - Disabled  
- IOMMU - Enabled
- WakeUp USB - Enabled
- 4G - Enabled
...
### Not Tested
- USB ports - I have not tested all ports
...
The first step is to put all beer in the fridge and then download OpenCore [package](https://github.com/acidanthera/opencorepkg/releases).
There are a lot of videos on YouTube on how to generate a new `config.plist`. The simplest way is to copy the Sample.plist file from `..\OpenCore-1.0.3-RELEASE\Docs` in `..\OpenCore-1.0.3-RELEASE\X64\EFI\OC\` and change the name.

The next step is to download the recovery image of mac os you want. You could use `macrecovery` tool located in OpenCore folder:
```sh 
...
For example, if you want mac OS Sonoma the command should look like:
```sh
macrecovery.py -b Mac-827FAC58A8FDFA22 -m 00000000000000000 download
```
Meanwhile, you could prepare your USB drive or take a beer. If you are using Windows download Rufus and format your USB drive with the following settings:
...
Once you have the recovery image on your computer copy the folder named: `com.apple.recovery.boot` also onto your USB drive.

Next, take another beer and start collecting all the needed information and configuration about your system:
...
- Download all kext files for your system - read the [Dortania Guide](https://dortania.github.io/OpenCore-Install-Guide/) carefully and take notes on what you need. *It will take three or more beers time.*

Now, put all your notes in `config.plist` There is more than one way to do but **never use text editor**. Both tools `OCAuxiliaryTools` and `ProperTree` are good choices. Check on YouTube how to use each of them. Personally, I started with ProperTree but then I switched to OCAuxiliaryTools because I can update the kext files easily and check for misconfigurations.
...
If you follow the instructions of other tools, all information about SMBIOS and USB ports should already be saved in config.plist file.

Grab a beer and go to the next step - edit **Kernel**
